...
WITH preferred_invoice_id AS (
    ...
    JOIN LATERAL
        jsonb_array_elements(inp.textract_json -> 'ExpenseDocuments') AS ed(value) ON true
    JOIN LATERAL
        jsonb_array_elements(ed.value -> 'SummaryFields') AS summary_field ON true
    GROUP BY inp.id
)
SELECT 
    inp.id AS in_invoice_processing_id,
    ...
    pi.invoice_receipt_id AS invoice_receipt_id
FROM 
    in_invoice_processing inp
JOIN LATERAL
...
JOIN preferred_invoice_id pi ON inp.id = pi.in_invoice_processing_id
